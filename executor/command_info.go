...
func (info *commandInfo) pOrderAppend(pOrder *protos.PartialOrder) {
	info.pOrders[pOrder.Author()] = pOrder
...
func (info *commandInfo) prioriRecord(priInfo *commandInfo) {
	info.priCmd[priInfo.curCmd] = true
}

func (info *commandInfo) prioriAppend(cmdMap map[string]bool) {
	for commandD := range cmdMap {
		info.priCmd[commandD] = true
	}
}

func (info *commandInfo) appendLow(commandD string) {
	info.lowCmd[commandD] = true
}

func (info *commandInfo) parentLow(parent *commandInfo) {
	for digest := range parent.lowCmd {
		info.lowCmd[digest] = true
	}
}
