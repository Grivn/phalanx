...
func testUSIGAuthenScheme(t *testing.T) {
	usig1, err := sgx.New(usigEnclaveFile, nil)
	require.NoError(t, err)

	pubKey := usig1.EnclavePublicKey()
	...
	usig2, err := sgx.New(usigEnclaveFile, usig1.EnclaveSealedKey())
	require.NoError(t, err)
	assert.Equal(t, pubKey, usig2.EnclavePublicKey())
