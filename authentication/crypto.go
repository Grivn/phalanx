...
func NewSGXUSIGAuthenticationScheme(usig api.USIG) *SGXUSIGAuthenticationScheme {
...
func (au *SGXUSIGAuthenticationScheme) VerifyAuthenticationTag(m []byte, sig []byte, pubKey interface{}) error {
	ui := &commonProto.UI{}
	if err := proto.Unmarshal(sig, ui); err != nil {
		au.logger.Errorf("Unmarshal USIG error: %s", err)
		return err
	}
	...
	usigID, err := sgx.MakeID(epoch, pubKey)
	if err != nil {
