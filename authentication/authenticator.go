...
func NewWithSGXUSIG(roles []api.AuthenticationRole, id uint32, keystoreFileReader io.Reader, enclaveFile string) (*Authenticator, error) {
	...
	usig, err := sgx.New(enclaveFile, usigKey.([]byte))
	if err != nil {
...
func NewWithUSIG(roles []api.AuthenticationRole, id uint32, ks BftKeyStorer, usig api.USIG) (*Authenticator, error) {
	return new(roles, id, ks, usig)
}

func new(roles []api.AuthenticationRole, id uint32, ks BftKeyStorer, usig api.USIG) (*Authenticator, error) {
	a := &Authenticator{
		id:          id,
		ks:          ks,
