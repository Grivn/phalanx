...
func (spec *sgxEcdsaKeySpec) generateKeyPair(securityParam int) (string, string, error) {
	usig, err := sgx.New(spec.enclaveFile, nil)
	if err != nil {
		return "", "", err
	}
	defer usig.EnclaveDestroy()

	privKeyBytes := usig.EnclaveSealedKey()
	pubKeyBytes, err := x509.MarshalPKIXPublicKey(usig.EnclavePublicKey())
	if err != nil {
