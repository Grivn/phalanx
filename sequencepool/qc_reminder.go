...
func (qr *qcReminder) restoreQCs(tracker CommandTracker) {
	...
	for {
		...
		tracker.Del(minQC.CommandDigest())
	}
}
...
func (qr *qcReminder) proposedQC(remoteQC *protos.QuorumCert) {
	// remove the remoteQC from cache.
	qr.cachedDelete(remoteQC)

	// update the proposedNo
	qr.proposedNo[remoteQC.Sequence()] = true
