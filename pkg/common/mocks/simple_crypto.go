...
func GenerateExternalCrypto(author uint64, count int) (external.PrivateKey, external.PublicKeys, error) {
	privateKey := &ecdsaP256PrivateKey{SignAlg: ECDSA_P256}
	publicKeys := &ecdsaP256PublicKeys{SignAlg: ECDSA_P256, PublicKeys: make(map[uint64]*ecdsaP256PublicKey)}
	for i := 0; i < count; i++ {
		id := uint64(i + 1)
		...
		publicKeys.PublicKeys[id] = &ecdsaP256PublicKey{SignAlg: ECDSA_P256, PublicKey: pair.PrivateKey.PublicKey}
		if id == author {
			privateKey = pair
		}
	}
	return privateKey, publicKeys, nil
}
...
func generateKeyHelper(signer string, id int) (*ecdsaP256PrivateKey, error) {
...
type ecdsaP256PublicKeys struct {
	SignAlg    string
	PublicKeys map[uint64]*ecdsaP256PublicKey
}
...
func (pubs *ecdsaP256PublicKeys) Algorithm() string {
	return pubs.SignAlg
}

func (pubs *ecdsaP256PublicKeys) Verify(id uint64, cert *protos.Certification, hash types.Hash) error {
	pub, ok := pubs.PublicKeys[id]
	if !ok {
		return fmt.Errorf("cannot find public key for node %d", id)
	}
	return pub.Verify(cert, hash)
}
