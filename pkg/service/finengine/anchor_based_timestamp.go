...
func (tab *timestampAnchorBasedOrdering) freeWill(frontStream types.FrontStream) ([]types.InnerBlock, uint64) {
	...
	for _, frontC := range frontStream.Stream {
		...
		rawCommand := tab.readCommand(frontC.Digest)
...
func (tab *timestampAnchorBasedOrdering) readCommand(commandD string) *protos.Command {
	command := tab.commandTracker.Get(commandD)
