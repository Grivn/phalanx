...
func NewMetaPool(conf config.PhalanxConf,
	privateKey external.PrivateKey, publicKeys external.PublicKeys,
	sender external.Sender, logger external.Logger,
	ms *metrics.Metrics) api.MetaPool {
	...
	pTracker := tracker.NewPartialTracker(conf.NodeID, logger)
	...
	for i := 0; i < conf.NodeCount; i++ {
		id := uint64(i + 1)
		subs[id] = instance.NewReplicaInstance(conf.NodeID, id, pTracker, cryptoService, sender, logger)
	...
	*active = int64(0)
	...
	for i := 0; i < conf.NodeCount*conf.Multi; i++ {
		id := uint64(i + 1)
		client := instance.NewClient(conf.NodeID, id, commandC, active, logger)
		clients[id] = client
	}
	...
	return &metaPool{
		...
		replicas: subs,
		pTracker: pTracker,
		cTracker: tracker.NewCommandTracker(conf.NodeID, logger),
		clients:  clients,
		commandC: commandC,
		timer:    timer.NewSingleTimer(conf.Timeout, logger),
		closeC:   make(chan bool),
		crypto:   cryptoService,
		sender:   sender,
		logger:   logger,
		metrics:  ms.MetaPoolMetrics,
		commitNo: committedTracker,
		active:   active,
		byz:      conf.IsByzantine,
		//snapping: true,
		//first:    true,
	}
}
